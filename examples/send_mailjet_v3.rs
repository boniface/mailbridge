use mailbridge::{
    EmailMessage, MailClient, MailbridgeConfig, MailjetApiVersion, MailjetConfig, MailjetProvider,
};

#[tokio::main]
async fn main() -> mailbridge::Result<()> {
    let config = MailbridgeConfig::from_env()?;
    let provider_config = MailjetConfig::from_env(&config)?.with_api_version(MailjetApiVersion::V3);
    ...
    let message = EmailMessage::builder()
        ...
        .subject("Mailjet v3 test")
        .text("Sent through Mailjet v3.")
        .build()?;
